...
Examples:
  ...
  python main.py --ar-demo                    # AR visualization demo
    ...
    parser.add_argument(
        "--live-localize",
        action="store_true",
        help="Real-time spatial visualization on camera feed (no recording needed)",
    )
    parser.add_argument(
        "--localize-duration",
        type=int,
    ...
    if len(sys.argv) == 1:
        logger.info("\n[DEFAULT] Running spatial visualization...")
        logger.info("-" * 70)
...
def run_live_localization(duration_sec=60):
    ...
    # Deprecated: use run_spatial_visualization instead
    logger = get_logger(__name__)
    logger.warning("run_live_localization is deprecated, using run_spatial_visualization instead")
    return run_spatial_visualization(duration_sec)
